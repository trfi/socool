...
const RegisterComponent = () => {
  ...
  const handleRegister = async (data) => {
    try {
      await axios.post('/api/register', { username: data.Username, email: data.Email, password: data.Password });
      router.push('/profile');
    } catch (err) {
      console.log(err);
    }
    ...
    <div className="flex justify-between my-20 text-gray-dark container mx-auto">
      <section className="flex flex-col bg-white w-5/12 mx-10 py-12 px-12 md:px-20 rounded-3xl text-center justify-center items-center space-y-4">
        ...
        <form onSubmit={handleSubmit(handleRegister)} className="space-y-4 w-full">
          <Input label="Username" register={register} placeholder required>
            <div className="absolute top-2 right-3"><Image src={identification} alt="identification" /></div>
          </Input>
